...
public class WeatherAPI implements IWeatherAPI {
    ...
    public Weather getWeatherByCity(int cityId) {
        ...
        try {
            ...
            {
                retval = new Weather(0, ow.id,ow.coord.lat, ow.coord.lon,ow.name,ow.weather.get(0).main, ow.weather.get(0).description, ow.weather.get(0).icon, ow.main.temp,ow.main.pressure, ow.main.humidity, ow.main.temp_min, ow.main.temp_max);
            }
            logger.debug(ow);
        } catch (IOException e) {
